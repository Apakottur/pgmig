...
SELECT
    ...
    -- while the whole type is missing (same phantom-row hazard tables.sql solves for
    -- zero-column tables). FILTER drops the LEFT JOIN's NULL placeholder, and COALESCE
    -- turns the empty group into an empty JSON array.
    COALESCE(jsonb_agg(jsonb_build_object('name', a.attname, 'type', format_type(a.atttypid, a.atttypmod))
        ORDER BY a.attnum) FILTER (WHERE a.attnum IS NOT NULL), '[]'::jsonb) AS fields
FROM
    pg_type t
