...
async def load(conn: psycopg.AsyncConnection[Any], db_info: DbInfo) -> None:
    ...
    """
    for row in await _run_query(conn, "view_dependencies.sql", _ViewDependencyRow):
...
async def check(conn: psycopg.AsyncConnection[Any]) -> list[str]:
    ...
    """
    findings: list[str] = []
    for row in await _run_query(conn, "matview_dependencies.sql", _MatviewDependencyRow):
