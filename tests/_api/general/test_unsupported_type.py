...
def test_instead_of_trigger_on_view_raises_not_supported(gen_setup: GenerateSetup) -> None:
    ...
    with pytest.raises(PgmigError, match=r"INSTEAD OF trigger .* is not supported"):
        generate(source=gen_setup.src.dsn, target=gen_setup.dst.dsn)
...
def test_rule_raises_not_supported(gen_setup: GenerateSetup) -> None:
    ...
    """
    gen_setup.dst.execute("CREATE TABLE t (n integer);\nCREATE RULE t_no_insert AS ON INSERT TO t DO INSTEAD NOTHING")

    with pytest.raises(PgmigError, match=r"rule .* is not supported"):
        generate(source=gen_setup.src.dsn, target=gen_setup.dst.dsn)
...
def test_rls_policy_raises_not_supported(gen_setup: GenerateSetup) -> None:
    ...
    with pytest.raises(PgmigError, match=r"row-level security policy .* is not supported"):
        generate(source=gen_setup.src.dsn, target=gen_setup.dst.dsn)
...
def test_rls_enabled_table_raises_not_supported(gen_setup: GenerateSetup) -> None:
    ...
    with pytest.raises(PgmigError, match=r"row-level security .* is not supported"):
        generate(source=gen_setup.src.dsn, target=gen_setup.dst.dsn)
...
def test_base_type_raises_not_supported(gen_setup: GenerateSetup) -> None:
    ...
    with pytest.raises(PgmigError, match=r"base type .* is not supported"):
        generate(source=gen_setup.src.dsn, target=gen_setup.dst.dsn)
...
def test_inheritance_child_raises_not_supported(gen_setup: GenerateSetup) -> None:
    ...
    """
    gen_setup.dst.execute("CREATE TABLE parent (n integer);\nCREATE TABLE child () INHERITS (parent)")

    with pytest.raises(PgmigError, match=r"inheritance child .* is not supported"):
        generate(source=gen_setup.src.dsn, target=gen_setup.dst.dsn)
...
def test_extended_statistics_raises_not_supported(gen_setup: GenerateSetup) -> None:
    ...
    must raise.
    """
    gen_setup.dst.execute("CREATE TABLE t (a integer, b integer);\nCREATE STATISTICS t_stats ON a, b FROM t")

    with pytest.raises(PgmigError, match=r"extended statistics .* is not supported"):
        generate(source=gen_setup.src.dsn, target=gen_setup.dst.dsn)
...
def test_event_trigger_raises_not_supported(gen_setup: GenerateSetup) -> None:
    ...
    with pytest.raises(PgmigError, match=r'event trigger "et" is not supported'):
        generate(source=gen_setup.src.dsn, target=gen_setup.dst.dsn)
